<template>
  <div :class="{ 'active-menu-item': (model.id === currentSelectedNodeId)}">
    <!-- {{model.id}} - {{currentSelectedNodeId}} -->
      ...
      <div v-else>
        <div v-if="model.type=== 'product'&&model.currentUserVisible===false">
          <Submenu v-if="switchToShow===true" :name="model.id">
            ...
            <div v-if="isFolder">
              <MySubMenu v-for="item in model.children" :model="item" :currentSelectedNodeId="currentSelectedNodeId"></MySubMenu>
            </div>
          </Submenu>
        </div>
        <div v-else>
...
export default {
  name: 'MySubMenu',
  props: ['model', 'currentSelectedNodeId'],
